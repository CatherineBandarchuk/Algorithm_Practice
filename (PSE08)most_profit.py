# You are given an integer array prices where prices[i] is the price of a 
# given stock on the ith day.
# On each day, you may decide to buy and/or sell the stock. You can only 
# hold at most one share of the stock at any time. However, you can buy it 
# then immediately sell it on the same day.

# Find and return the maximum profit you can achieve.

# Example 1:
# Input: prices = [7,1,5,3,6,4]
# Output: 7
# Explanation: Buy on day 2 (price = 1) and sell on day 3
#   (price = 5), profit = 5-1 = 4.
# Then buy on day 4 (price = 3) and sell on day 5 
#   (price = 6), profit = 6-3 = 3.
# Total profit is 4 + 3 = 7.
# 
# Example 2:
# Input: prices = [1,2,3,4,5]
# Output: 4
# Explanation: Buy on day 1 (price = 1) and sell on 
#    day 5 (price = 5), profit = 5-1 = 4.
# Total profit is 4.

def maximum_profit(number_list):
    if not isinstance(number_list, list) \
            or not all(isinstance(element, (int)) for element in number_list):
        return -1
    
    if len(number_list) == 0:
        return 0
    
    profit = 0
    buy_index = 0
    sell_index = 0 
    for i in range(len(number_list)):
        
        if number_list[i] < 0:
            return -1

        if number_list[i] == 0:
            return profit

        if i + 1 < len(number_list):
            if number_list[i] < number_list[i + 1]:
                sell_index += 1 
            else: 
                profit += (number_list[sell_index] - number_list[buy_index])
                buy_index = i + 1
                sell_index = buy_index 
        else:
            if sell_index!= buy_index:
                profit += (number_list[sell_index] - number_list[buy_index])
    
    return profit

prices1 = [7,1,5,3,6,4]
#print(f"prices: {prices1}\nprofit: {maximum_profit(prices1)}") # 7

prices2 = [1,2,3,4,5]
#print(f"prices: {prices2}\nprofit: {maximum_profit(prices2)}") # 4

prices3 = [7,6,4,3,1]
#print(f"prices: {prices3}\nprofit: {maximum_profit(prices3)}") # 0 

prices4 = [1,1,4,4,7,5]
#print(f"prices: {prices4}\nprofit: {maximum_profit(prices4)}") # 6

prices5 = [7,1,5,3,6,4]
print(f"prices: {prices5}\nprofit: {maximum_profit(prices5)}") # 7